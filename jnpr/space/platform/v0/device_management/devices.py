...
class Device(resource.Resource):
    ...
    def __init__(self, rest_end_point=None, xml_data=None, attrs_dict=None):
        ...
        self._media_type = "application/vnd.net.juniper.space."\
        "device-management.device+xml;version=1;charset=UTF-8"

    def __str__(self):
        s = "Device <name=%s, ip=%s, model=%s, "\
        "family=%s, version=%s, serialnum=%s>" % \
          (self.name, self.ipAddr, self.platform, self.deviceFamily,
           self.OSVersion, self.serialNumber)
        return s
...
class DeviceCollection(collection.Collection):
    ...
    def __init__(self, rest_end_point, parent):
        super(DeviceCollection, self).__init__(rest_end_point,
                                               'devices',
...
class DeviceManager(service.Service):
