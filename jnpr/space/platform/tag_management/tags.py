...
class Tag(resource.Resource):
    ...
    def __init__(self, rest_end_point=None, tag_xml=None, attrs_dict=None):
        ...
        self._xml_name = "tag"
        self._media_type = "application/vnd.net.juniper.space.tag-management."\
        "tag+xml;version=1;charset=UTF-8"
        self._init_targets()

    def _init_targets(self):
        try:
            targets_href = self.get_href() + "/targets"
            self._collections['targets'] = TargetCollection(
...
class TagCollection(collection.Collection):
    """Represents a collection of Tags"""

    def __init__(self, rest_end_point, parent):
        super(TagCollection, self).__init__(rest_end_point, 'tags',
                                            '/api/space/tag-management/tags',
                                            parent)
        self._xml_name = "tags"
        self._media_type = "application/vnd.net.juniper.space.tag-management."\
        "tags+xml;version=1;charset=UTF-8"
...
class Target(resource.Resource):
    ...
    def __init__(self, rest_end_point=None, xml_data=None, attrs_dict=None):
        super(Target, self).__init__(rest_end_point, xml_data, attrs_dict)
        self._service_url = TagManager.service_url
        self._collection_name = "targets"
        self._xml_name = "target"
        self._media_type = "application/vnd.net.juniper.space.tag-management."\
        "target+xml;version=1;charset=UTF-8"

    def __str__(self):
        s = "Target <uri='%s', href='%s', type=%s>" % (self.uri,
                                                       self.href, self.type)
        return s
...
class TargetCollection(collection.Collection):
    """Represents a collection of Targets"""

    def __init__(self, rest_end_point, href, parent):
        super(TargetCollection, self).__init__(rest_end_point, 'targets',
                                               href, parent)
        self._xml_name = "targets"
        self._media_type = "application/vnd.net.juniper.space.tag-management."\
        "targets+xml;version=1;charset=UTF-8"

    def _create_resource(self, xml_data):
        return Target(self._rest_end_point, xml_data)

class TagManager(service.Service):
